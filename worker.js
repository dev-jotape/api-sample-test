...
/**
 * Get recently modified meetings and process their associations
 */
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    const contactAssociationsResults = (await (await hubspotClient.apiRequest({
      ...
      body: { inputs: meetingsToAssociate.map(contactId => ({ id: contactId })) }
    })).json())?.results || [];

    data.forEach(meeting => {
      if (!meeting.properties) return;

      const contact = contactAssociationsResults.find(a => a.from.id === meeting.id)?.to[0].id;
